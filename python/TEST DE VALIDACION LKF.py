"""
TEST DE VALIDACIÓN IBI - LKF CON D_KL Y TRAYECTORIA GEODÉSICA
Versión: 6.0 (Basado en IBI_TEST_3GRG.py + TEST DE VALIDACION AKF.py)
Fecha: Marzo 2026
...
trayectoria_geodesica_TIMESTAMP.png (Figura 4)
dkl_por_iteracion_TIMESTAMP.csv (Datos para análisis)

Carpeta de salida: test_ibi_lkf
"""
import numpy as np
...
#================================================================================
#FILTRO DE KALMAN LINEAL (LKF) CON H CORREGIDA
#================================================================================
class LinearPhysicsInformedKalmanFilter:
    """
    Filtro de Kalman Lineal - R y Q fijos (NO adaptativo)
    CORRECCIÓN: El manuscrito especifica que LKF mantiene R y Q fijos
    para garantizar estabilidad numérica y mínimo costo computacional.
    """
    ...
    def __init__(self, dt=0.075, R_base=1e-5):
        ...
        # Ruido de proceso (Q fijo - NO adaptativo en LKF)
        self.Q = np.diag([1e-6, 1e-7, 1e-5])
        self.R = np.array([[R_base]])

    def filter(self, measurements, track_dkl=True):
        """
        Aplicar filtro de Kalman Lineal completo con seguimiento de D_KL
        ...
        # NUEVO: Historiales para análisis de información
        ...
        for k in range(1, n_meas):
            ...
            # ========== INNOVACIÓN (RESIDUO) ==========
            z_k = measurements[k]
            residuo = z_k - (self.H @ x_pred)
            
            # ========== ACTUALIZACIÓN CON R FIJO (LKF) ==========
...
class IBIStressTestStable:
    """Test de validación con Filtro Kalman Lineal (LKF) + D_KL"""
    ...
    def __init__(self, config=TEST_CONFIG):
        ...
        # LKF con parámetros fijos
        self.kalman_filter = LinearPhysicsInformedKalmanFilter(
            dt=self.dt, 
            R_base=1e-5
        )
    ...
    def kalman_inference_engine(self, raw_signal, track_dkl=True):
        """Motor Kalman Lineal con seguimiento de D_KL"""
    ...
    def verify_observability_matrix(self):
        ...
        print("\n" + "="*80)
        print("VERIFICACIÓN DE OBSERVABILIDAD (LKF)")
        print("="*80)
    ...
    def compare_variance_reduction(self, n_samples_range=[10, 50, 100, 200, 500]):
        ...
        CORRECCIÓN: Limitar ventana a zona estable para evitar que el drift infle artificialmente el ratio.
        """
        print("\n" + "="*80)
        print("COMPARACIÓN DE REDUCCIÓN DE VARIANZA (LKF)")
        print("="*80)
        ...
        for n_samp in n_samples_range:
            ...
            for _ in range(50):
                ...
                y_lkf, _, _, _ = self.kalman_inference_engine(y_noisy, track_dkl=False)
                
                err_avg = y_avg[:self.time_interference] - y_clean[:self.time_interference]
                err_lkf = y_lkf[:self.time_interference] - y_clean[:self.time_interference]
                
                variances_avg.append(np.var(err_avg))
                variances_lkf.append(np.var(err_lkf))
            
            mean_var_avg = np.mean(variances_avg)
            mean_var_lkf = np.mean(variances_lkf)
            ratio = mean_var_avg / mean_var_lkf if mean_var_lkf > 1e-15 else np.inf
            
            results_by_samples.append({
                'n_samples': n_samp,
                'var_avg': mean_var_avg,
                'var_lkf': mean_var_lkf,
                'ratio': ratio
            })
            
            print(f"\n  N={n_samp:3d}: Var(Avg)={mean_var_avg:.6e}, Var(LKF)={mean_var_lkf:.6e}, Ratio={ratio:.2f}×")
    ...
    def plot_geodesic_trajectory(self, output_dir='./test_ibi_lkf'):
        """
        ...
        plt.title('Trayectoria Geodésica del LKF en el Manifold de Fisher\n' +
    ...
    def plot_convergence_mse(self, output_dir='./test_ibi_lkf'):
        ...
        err_lkf = (y_lkf - y_clean)**2
        
        plt.figure(figsize=(12, 6))
        plt.plot(self.t * 1000, err_avg, color='orange', alpha=0.4, label='Error Promediado Estándar')
        plt.plot(self.t * 1000, err_lkf, color='green', linewidth=2, label='Error Filtro LKF (Propuesto)')
        
        plt.yscale('log')
        plt.xlabel('Tiempo de Adquisición (ms)', fontsize=12)
        plt.ylabel('MSE Instantáneo [log]', fontsize=12)
        plt.title('Convergencia de Error en Tiempo Real (Promediado vs. LKF)', fontsize=14, fontweight='bold')
        plt.legend()
    ...
    def plot_variance_reduction(self, output_dir='./test_ibi_lkf'):
        ...
        plt.title('Ratio de Reducción de Varianza (LKF)', fontsize=13)
    ...
    def plot_signal_resilience(self, output_dir='./test_ibi_lkf'):
        ...
        plt.plot(self.t, y_lkf, color='blue', label='Señal Filtrada (IBI-LKF)', linewidth=2.5)
        
        plt.title("Resiliencia y Filtrado de Señal IBI (LKF)", fontsize=15, fontweight='bold')
    ...
    def print_summary(self):
        ...
        print("\n" + "="*80)
        print("RESULTADOS ESTADÍSTICOS (LKF)")
        print("="*80)
        ...
        print(f"  Filtro: {self.results.get('filter_type', 'LKF')}")
        print("="*80)
...
if __name__ == "__main__":
    print("="*80)
    print("STRESS-TEST IBI - LKF CON D_KL Y TRAYECTORIA GEODÉSICA")
    print("Versión 6.0 - Linear Kalman Filter")
    print("="*80)
