"""
TEST DE VALIDACIÓN IBI - EKF CON D_KL Y TRAYECTORIA GEODÉSICA
Versión: 9.0 (Adaptado de UKF a EKF)
Fecha: Marzo 2026
Autor: Daniel Vivas - dvivas1@uc.edu.ve
Adaptación: UKF → EKF (Extended Kalman Filter)
"""
import numpy as np
...
#================================================================================
class ExtendedPhysicsInformedKalmanFilter:
    """
    Filtro de Kalman Extendido - Linearización mediante Jacobianos
    Para sistemas no-lineales: x_k = f(x_{k-1}) + w, z_k = h(x_k) + v
    """
    ...
    def __init__(self, dt=0.075, R_base=1e-5):
        ...
        # Matriz de observación (lineal en este caso)
        self.H = np.array([[1.0, 0.1, 0.05]])

    def state_transition(self, x):
        """
        Función de transición de estado no-lineal
        x_new = F @ x (en este caso es lineal, pero usamos la forma general)
        """
        F = np.array([
            [1.0,   self.dt, 0.1],
            [0.0,   0.95,    0.0],
    ...
    def jacobian_state_transition(self, x):
        ...
        F = df/dx evaluada en x
        Para nuestro modelo lineal, es constante
        """
        F = np.array([
            [1.0,   self.dt, 0.1],
            [0.0,   0.95,    0.0],
    ...
    def jacobian_observation(self, x):
        ...
        H = dh/dx evaluada en x
        Para h(x) = x[0] + 0.1*x[1] + 0.05*x[2], es constante
        """
        return np.array([[1.0, 0.1, 0.05]])

    def filter(self, measurements, track_dkl=True):
        """
        Aplicar filtro EKF completo con seguimiento de D_KL
        ...
        I_F_history : list (opcional)
            Matrices de Fisher por iteración
        """
        ...
        # Historiales para análisis de información
        ...
        for k in range(1, n_meas):
            ...
            # Linearizar alrededor del estado estimado anterior
            ...
            # Linearizar la función de observación
            ...
            # ========== CÁLCULO DE D_KL ==========
            if track_dkl:
                # Distribución del sensor (P)
                p_mean = measurements[k]
                p_var = p_var_initial
                
                # Distribución del filtro (Q)
                q_mean = x_upd[0]
                q_var = P[0, 0]
...
class IBIStressTestStable:
    """Test de validación con Filtro Kalman Extendido (EKF) + D_KL"""
    ...
    def __init__(self, config=TEST_CONFIG):
        ...
        self.kalman_filter = ExtendedPhysicsInformedKalmanFilter(
            dt=self.dt, 
            R_base=1e-5
        )
    ...
    def kalman_inference_engine(self, raw_signal, track_dkl=True):
        """Motor EKF con seguimiento de D_KL"""
    ...
    def verify_observability_matrix(self):
        ...
        print("\n" + "="*80)
        print("VERIFICACIÓN DE OBSERVABILIDAD (EKF)")
        print("="*80)
        
        n = self.kalman_filter.n_states
        F = self.kalman_filter.jacobian_state_transition(np.zeros(n))
    ...
    def compare_variance_reduction(self, n_samples_range=[10, 50, 100, 200, 500]):
        ...
        print("\n" + "="*80)
        print("COMPARACIÓN DE REDUCCIÓN DE VARIANZA (EKF)")
        print("="*80)
        ...
        for n_samp in n_samples_range:
            ...
            for _ in range(50):
                ...
                y_ekf, _, _, _ = self.kalman_inference_engine(y_noisy, track_dkl=False)
                
                err_avg = y_avg[:self.time_interference] - y_clean[:self.time_interference]
                err_ekf = y_ekf[:self.time_interference] - y_clean[:self.time_interference]
                
                variances_avg.append(np.var(err_avg))
                variances_ekf.append(np.var(err_ekf))
            
            mean_var_avg = np.mean(variances_avg)
            mean_var_ekf = np.mean(variances_ekf)
            ratio = mean_var_avg / mean_var_ekf if mean_var_ekf > 1e-15 else np.inf
            
            results_by_samples.append({
                'n_samples': n_samp,
                'var_avg': mean_var_avg,
                'var_ekf': mean_var_ekf,
                'ratio': ratio
            })
            
            print(f"\n  N={n_samp:3d}: Var(Avg)={mean_var_avg:.6e}, Var(EKF)={mean_var_ekf:.6e}, Ratio={ratio:.2f}×")
    ...
    def plot_geodesic_trajectory(self, output_dir='./test_ibi_ekf'):
        ...
        plt.title('Trayectoria Geodésica del EKF en el Manifold de Fisher\n' +
    ...
    def plot_convergence_mse(self, output_dir='./test_ibi_ekf'):
        ...
        err_ekf = (y_ekf - y_clean)**2
        
        plt.figure(figsize=(12, 6))
        plt.plot(self.t * 1000, err_avg, color='orange', alpha=0.4, label='Error Promediado Estándar')
        plt.plot(self.t * 1000, err_ekf, color='purple', linewidth=2, label='Error Filtro EKF (Propuesto)')
        
        plt.yscale('log')
        plt.xlabel('Tiempo de Adquisición (ms)', fontsize=12)
        plt.ylabel('MSE Instantáneo [log]', fontsize=12)
        plt.title('Convergencia de Error en Tiempo Real (Promediado vs. EKF)', fontsize=14, fontweight='bold')
        plt.legend()
    ...
    def plot_variance_reduction(self, output_dir='./test_ibi_ekf'):
        ...
        plt.title('Ratio de Reducción de Varianza (EKF)', fontsize=13)
    ...
    def plot_signal_resilience(self, output_dir='./test_ibi_ekf'):
        ...
        plt.plot(self.t, y_ekf, color='blue', label='Señal Filtrada (IBI-EKF)', linewidth=2.5)
        
        plt.title("Resiliencia y Filtrado de Señal IBI (EKF)", fontsize=15, fontweight='bold')
    ...
    def print_summary(self):
        ...
        print("\n" + "="*80)
        print("RESULTADOS ESTADÍSTICOS (EKF)")
        print("="*80)
        ...
        print(f"  Filtro: {self.results.get('filter_type', 'EKF')}")
        print("="*80)
...
if __name__ == "__main__":
    print("="*80)
    print("STRESS-TEST IBI - EKF CON D_KL Y TRAYECTORIA GEODÉSICA")
    print("Versión 9.0 - Extended Kalman Filter")
    print("="*80)
